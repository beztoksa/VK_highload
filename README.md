...
| **`address_messages`**  | Связь письма с адресами и ролями, а также папка письма | id(8) + message_id(8) + address_id(8) + role(1) + folder_id(8) + is_read(1) + is_starred(1) + is_important(1) + created_at(8) = 50 Б | 275 млрд         | 50 Б × 275 000 000 000 = 17 ТБ           | 2 818 + 8 054 + 5 236 = 16 108                          | 3 100 × 3 + 6 443 = 9 300 + 6 443 = 15 743                                   |
...
| **`address_messages`**  | Большая часть нагрузки приходится на ключ `address_id`.                                                                                              |
...
| **`address_messages`** | `folder_id`, `recived_at`      | B-Tree                       | сортировка фильтрация                                                     |
...
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|----------------------------------------------------|
| `users`, <br/>`addresses`, <br/>`messages`, <br/>`address_messages`, <br/>`folders`,  <br/>`threads`, <br/> `filters` | PostgreSQL                       | Нужны транзакции                                   | 
...
| **`threads`**                                    | По `id`                                                                                            | Синхронная репликация внутри шарда                       |
...
1. Группировка полученных сообщений по тредам.\
2. По первому сообщений находится id треда по `tread_id` в таблице **messages**\
Далее два варианта:\
Привью треда:\
3. В таблице находится денормализованное поле `last_message_id`.\
4. Если оно доступно для пользователя привью становиться это сообщения
если нет, проходясь по всем сообщениям треда находится первое доступное
Построения треда: \
3. Далее находятся все недостающие сообщения треда по `message_id` в таблице **treads** \
4. Фильтруются с помощью таблицы **address_messages** сообщения не уделанные и доступные пользователю \
5. Тред передается списком каждый элемент содержит время получения, id сообщения и id родительского сообщения, \
что позволяет построить на стороне frontend линейное или древовидное представление.
...
### 7.9 Влияние выбранного решения на физическую схему БД, профиль нагрузки и взаимодействие компонентов системы должно быть учтено в соответствующих разделах РПЗ
