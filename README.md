...
## 9. Схема проекта 
![Cхема сервисов](images/service_scheme.png)
...
|:--------------------|:--------------------------------------------------------------|
| auth                | Авторизация                                                   |
| API Gateway         | Обработка API запросов поход в узкоспециализированные сервисы |
| message reader      | Чтение одного сообщения                                       |
| mail processor      | Главный обработчик записи метаданных писем                    |
| search mail         | Сервис для обработки поисковых запросов                       |
| search index worker | Обновление индексов для поиска                                |
| SMTP convertor      | конвертация в smtp формат и обратно                           |
| out SMTP            | Отдача, упаковка внешних писем                                |
| in SMTP             | Прием, обработка внешних писем                                |
| antispam filter     | проверка писем на спам                                        |
...
|:------------------|:----------------------------------------------------------------------------------------------|
| **Nginx**         | Резервирование N+1                                                                            |
| **PostgreSQL**    | Aсихнронная репликация                                                                        |
| **Apache Kafka**  | Самописный механизм ретраев                                                                   |
| **S3**            | Настройка репликации файлов в разных стойках/ДЦ                                               |
| **Микросервисы**  | k8s, Graceful Shutdown, Degradation, асинхронная обработка событий, распределенные транзакции |

## Схема резервирования внутри SeaweedFS


| Уровень | Функция                                  | Механизм резервирования                                                             |
|---------|------------------------------------------|-------------------------------------------------------------------------------------|
| Master  | отвечает за резервирование между volumes | При отключении leader сервера, на его место встает один из follower серверов        |
| Volume  | отвечает за хранение копии данных        | Master сервер осуществляет выбранный механизм резервирования между volume синхронно |

### Выбор механизма резервирования volumes 
для каждого volume можно указать `-datacenter="dc1"` и `-rack="rack1"`, то есть резервирование может осуществляться на трех уровнях:
`-defaultReplication=xyz`

| Column | Meaning                                   |
|--------|-------------------------------------------|
| x	     | кол-во реплик в разных дц                 |
| y	     | кол-во реплик в разных стойках в одном дц |
| z	     | кол-во реплик в одной стойке              |
Репликация производится синхронно.
# 11. Расчет ресурсов
### Производительность на ядро 
| Технология | Характер сервиса      | RPS | RAM (ГБ) |
|:-----------|:----------------------|:---:|:--------:|
| Nginx      | SSL handshake (CPS)   | 500 |    10    |
| GO         | легкое JSON API       | 100 |   0,2    |
| GO         | средняя бизнес-логика | 100 |   0,7    |
| GO         | тяжелая бизнес-логика | 20  |    2     |
### Ресурсы на сервисы 
| Сервис              | Назначение                                                     | Пиковая нагрузка | CPU | RAM гб |
|---------------------|----------------------------------------------------------------|------------------|-----|--------|
| auth                | Авторизация                                                    | 23 630           | 24  | 5      |
| API Gateway         | Обработка API запросов: поход в узкоспециализированные сервисы | 23 630           | 24  | 5      |
| message reader      | Чтение одного сообщения                                        | 5 236            | 53  | 37     |
| mail processor      | Главный обработчик записи метаданных писем                     | 3 100            | 31  | 22     |
| search mail         | Сервис для обработки поисковых запросов                        | 802              | 9   | 6      |
| search index worker | Обновление индексов для поиска                                 | 3 100            | 155 | 310    |
| SMTP convertor      | Конвертация в SMTP-формат и обратно                            | 5 835            | 292 | 584    |
| out SMTP            | Отдача, упаковка внешних писем                                 | 2 170            | 22  | 16     |
| in SMTP             | Прием, обработка внешних писем                                 | 3 665            | 37  | 26     |
| antispam filter     | Проверка писем на спам                                         | 3 665            | 37  | 26     |
| Итог                |                                                                | 74833,6          | 684 | 1037   |


| База данных / Хранилище      | Размер таблицы (ТБ) | Нагрузка на чтение (QPS) | Нагрузка на запись (QPS) |
|------------------------------|--------------------:|-------------------------:|-------------------------:|
| PostgreSQL messages metadata |                 100 |                   60 840 |                   33 944 |
| ElasticSearch search index   |              40 000 |                      802 |                    9 543 |
| Redis sessions               |                 0,1 |                    2 818 |                      141 |
| PostgreSQL users addresses   |                 0,5 |                   19 728 |                    1 095 |
| S3 body attachments          |             150 000 |                   22 146 |                   12 406 |
| S3 temp SMTP messages        |                  10 |                   22 146 |                   12 406 |
| **Итог**                     |         **200 000** |              **128 480** |               **69 535** |

| Сервис     | Пиковая нагрузка       | CPU | RAM (ГБ) |
|------------|------------------------|----:|---------:|
| nginx      | 77 Гбит/с (21 284 RPS) |  80 |       80 |
| kafka      | 3 100 RPS              |   6 |       16 |
| smtp kafka | 5 835 RPS              |   8 |       16 |

### Расчет стоимости
| Название                     | Хостинг | Конфигурация                                                | Ядра | Кол-во | Цена $ | Аренда/мес $ |   Стоимость $ |
|:-----------------------------|:--------|:------------------------------------------------------------|-----:|-------:|-------:|-------------:|--------------:|
| kubenode (Go-сервисы)        | own     | 2×AMD EPYC 7543 / 8×32ГБ / 2×NVMe 4ТБ / 2×25GbE             |   64 |     10 |  6 000 |        5 000 |        60 000 |
| nginx-l7                     | own     | 1×Intel Xeon Silver 4314 / 2×32ГБ / 2×NVMe 512ГБ / 2×100GbE |   16 |      4 |  4 000 |          600 |        16 000 |
| db-postgres (metadata+users) | own     | 2×Intel Xeon Silver 4314 / 16×32ГБ / 4×NVMe 4ТБ / 2×25GbE   |   32 |     20 |  7 000 |        6 400 |       140 000 |
| db-ES (search index)         | own     | 2×Intel Xeon Silver 4314 / 8×32ГБ / 8×NVMe 4ТБ / 2×25GbE    |   32 |    100 | 15 000 |       50 000 |     1 500 000 |
| db-redis                     | own     | 2×Intel Xeon Silver 4314 / 16×32ГБ / 2×NVMe 512ГБ / 2×25GbE |   16 |      8 |  7 500 |        2 000 |        60 000 |
| kafka                        | own     | 2×Intel Xeon Silver 4314 / 4×32ГБ / 6×NVMe 4ТБ / 2×25GbE    |   16 |      5 |  6 500 |        1 500 |        32 500 |
| s3                           | own     | 2×Intel Xeon Silver 4314 / 8×32ГБ / 40×20TB HDD / 2×25GbE   |  192 |    300 | 22 000 |      300 000 |     6 600 000 |
| **Итог**                     |         |                                                             |      |        |        |  **365 500** | **8 408 500** |

### Таблица ресурсов сервисов 
| Сервис              | CPU requests | CPU limits | RAM requests | RAM limits гб | Кол-во реплик |
|---------------------|--------------|------------|--------------|---------------|---------------|
| auth                | 24           | 24         | 4            | 5             | 4             |
| API Gateway         | 24           | 24         | 4            | 5             | 4             |
| message reader      | 53           | 53         | 29           | 37            | 4             |
| mail processor      | 31           | 31         | 17           | 22            | 12            |
| search mail         | 9            | 9          | 4            | 6             | 24            |
| search index worker | 155          | 155        | 248          | 310           | 3             |
| SMTP convertor      | 292          | 292        | 467          | 584           | 10            |
| out SMTP            | 22           | 22         | 12           | 16            | 3             |
| in SMTP             | 37           | 37         | 20           | 26            | 3             |
| antispam filter     | 37           | 37         | 20           | 26            | 3             |
